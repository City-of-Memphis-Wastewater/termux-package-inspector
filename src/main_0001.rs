...
impl PackageList {
    ...
    fn fetch_package_details(&self, package_name: &str) -> Package {
        ...
        if let Ok(output) = output {
            ...
            for line in stdout.lines() {
                ...
                if parts.len() == 2 {
                    match parts[0].trim() {
                        "Version" => package.version = parts[1].trim().to_string(),
                        "Maintainer" => package.maintainer = Some(parts[1].trim().to_string()),
                        "Installed-Size" => package.installed_size = Some(parts[1].trim().to_string()),
                        "Depends" => package.depends = Some(parts[1].trim().to_string()),
                        "Homepage" => package.homepage = Some(parts[1].trim().to_string()),
                        "Description" => package.description = Some(parts[1].trim().to_string()),
                        _ => {}
                    }
                }
            }
...
impl App {
    ...
    fn ui(&mut self, f: &mut Frame) {
        ...
        let detail = if let Some(i) = self.package_list.state.selected() {
            ...
            let detailed_pkg = self.package_list.fetch_package_details(&pkg.name);
            format!(
                "Name: {}\nVersion: {}\nMaintainer: {}\nInstalled-Size: {}\nDepends: {}\nHomepage: {}\nDescription: {}",
                detailed_pkg.name,
                detailed_pkg.version,
                detailed_pkg.maintainer.as_deref().unwrap_or("N/A"),
                detailed_pkg.installed_size.as_deref().unwrap_or("N/A"),
                detailed_pkg.depends.as_deref().unwrap_or("N/A"),
                detailed_pkg.homepage.as_deref().unwrap_or("N/A"),
                detailed_pkg.description.as_deref().unwrap_or("N/A")
            )
        } else {
